...
/** Customizations */
var call_button_text = 'Gọi';      // text showed on call button
var hangup_button_text = 'Tắt máy';      // text showed on hangup button
...
var button_minwidth = 70;               // if width is less then this value, then show icon instead of text
...
webphone_api.onCallStateChange(function (status, direction, peername, peerdisplayname, line)
{
    ...
    if (status === 'setup')
    {
        ...
        console.error("thời gian gọi :" + timef.getHours() + ": " + timef.getMinutes() + ": " + timef.getSeconds());
        lastoop = 1;
    ...
    else if (status === 'disconnected')
    {
        ...
        let dem = (timel - timef.getTime()) / 1000;
        let sip = webphone_api.setparameter('callto');
        
        let strurl = _id != null ? "Home/_update?stt=" + _id + "&Time=" + timef.getHours() + "h" + timef.getMinutes() + "m" + timef.getSeconds() + "s" + "&second=" + dem.toFixed(0) : "Home/_update?stt=new&sip=" + _sip + "&Time=" + timef.getHours() + "h" + timef.getMinutes() + "m" + timef.getSeconds() + "s" + "&second=" + dem.toFixed(0);
        alert(strurl);
            $.ajax({
                type: "GET",
                url: strurl,
        ...
        //console.error("/index?stt=" + _stt + "&Time=" + timef + "&second=" + (int)((timel - timef) / 1000));
...
// get callto from DIV title - used in case multiple click to call  buttons are on a webpage
    ...
    if (webphone_api.isincall() === false) // make a call
    {
        ...
        }else
        {
            ...
            SetButtonState(lastnr, true);
            DisplayStatus('Khởi tạo cuộc gọi');
...
webphone_api.onAppStateChange(function (state)
{
    ...
    webphone_api.GetDisplayableNotifications(function (msg)
    {
        ...
        console.error('tin nhắn trả về : ' + msg )
        DisplayStatus(msg);
    });
...
function Btnc2kMousedown(ev, nr) // handle right click
{
    ...
    console.log("thowif gian ghi nhanj"+longclick_initiated);
    return false;
}
